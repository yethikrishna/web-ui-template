...
const STATS_URL = "https://assets.myndlabs.tech/stats/stats.json";
const REPO_API_URL = "https://api.github.com/repos/yethikrishna/web-ui-template";
...
function timelineTerminal(stats: StatsJson, milestones: Milestone[]): string {
  ...
  return `
    <div class="milestone-term" role="img" aria-label="varsha timeline">
      <div class="term-line term-cmd-line"><span class="term-prompt">$</span><span class="term-cmd">varsha stats --timeline</span></div>
      ${body}
    </div>`;
}
...
function render(stats: StatsJson): void {
  ...
  // traffic
  setText("traffic-sub", `myndlabs.tech traffic over the last ${stats.traffic.windowDays} days, measured by Cloudflare.`);
  setHtml(
    "rank-countries",
    rankList(
